...
import { Config, ConfigProvider, Context, Duration, Effect, Layer, Schedule, Schema } from 'effect';
import { HttpApiEndpoint, HttpApiGroup } from 'effect/unstable/httpapi';
import { HttpServerResponse } from 'effect/unstable/http';

type ScheduledEvent = Readonly<{ id: string; name: string; description: string | null }>;

const SyncResponse = Schema.Struct({ success: Schema.Boolean, message: Schema.String });
...
export const CalendarTriggerLive = Layer.effect(
	...
	Effect.gen(function* () {
		...
		const icsUrl = yield* Config.withDefault(Config.string('CALENDAR_ICS_URL'), '');
		...
		const doSync = Effect.gen(function* () {
			...
			const parsed = yield* Effect.tryPromise(() => ical.async.fromURL(icsUrl));

			const calendarEvents = Object.values(parsed)
				.filter((e) => e !== undefined)
				// eslint-disable-next-line @typescript-eslint/no-explicit-any -- node-ical types lack VEvent-specific fields
			...
			const existing = yield* rest.listGuildScheduledEvents(guildId).pipe(
			...
			for (const ev of existing) {
				...
				if (match?.[1]) descIndex.set(match[1], ev);
			}
			...
			for (const ce of calendarEvents) {
				...
				if (!existingEntry) {
					...
					if (needsUpdate) {
						...
						yield* rest.updateGuildScheduledEvent(guildId, existingEntry.id, {
							name: ce.name, description: ce.description,
							scheduled_start_time: ce.start.toISOString(),
							scheduled_end_time: ce.end.toISOString(),
							entity_metadata: { location: ce.location },
			...
			for (const [key, entry] of descIndex) {
				if (!knownKeys.has(key)) {
					...
					yield* rest.deleteGuildScheduledEvent(guildId, entry.id).pipe(Effect.catch(() => Effect.void));
				}
			}
		...
		return { trigger: doSync.pipe(Effect.catch((err) => Effect.logError('calendarSync: sync error', err))) };
...
export const CalendarSyncLive = Layer.effectDiscard(
	Effect.gen(function* () {
		...
		const intervalMinutes = yield* Config.withDefault(Config.int('CALENDAR_SYNC_INTERVAL_MINUTES'), 10);
		const icsUrl = yield* Config.withDefault(Config.string('CALENDAR_ICS_URL'), '');
		const guildId = yield* Config.withDefault(Config.string('GUILD_ID'), '');
		...
		yield* Effect.forkDetach(
			trigger.trigger.pipe(Effect.repeat(Schedule.fixed(Duration.minutes(intervalMinutes)))),
		);
	}).pipe(Effect.provideService(ConfigProvider.ConfigProvider, ConfigProvider.fromEnv())),
);
	...
	return Effect.gen(function* () {
		...
		return handlers.handle('triggerSync', () =>
			Effect.gen(function* () {
				...
				Effect.catch(() => Effect.succeed(toJson({ success: false, message: 'Sync failed' }, { status: 500 }))),
			),
		);
	}).pipe(Effect.provideService(ConfigProvider.ConfigProvider, ConfigProvider.fromEnv()));
}
