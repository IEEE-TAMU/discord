...
export function execute(interaction: ChatInputCommandInteraction) {
	...
	return Effect.gen(function* () {
		...
		yield* notes.add(card.value.id, userId, content);
		return `Note added to **#${card.value.id}: ${card.value.title}**.`;
	});
}
...
export function handleError(error: unknown): { content: string; flags: number } | string {
	...
	return { content: 'Failed to add note.', flags: MessageFlags.Ephemeral };
}
