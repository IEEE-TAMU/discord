...
export function execute(interaction: ChatInputCommandInteraction) {
	...
	return Effect.gen(function* () {
		...
		if (Option.isNone(remindAt)) {
			return yield* new InvalidDate({ input: timeStr });
		}

		yield* reminders.create(card.value.id, userId, remindAt.value, message);
		return `Reminder set for **#${card.value.id}: ${card.value.title}** at ${remindAt.value.toLocaleString()}.`;
	});
}
...
export function handleError(error: unknown): { content: string; flags: number } | string {
	...
	if (error instanceof InvalidDate) {
		return { content: 'Invalid time format. Use relative (1h, 30m, 2d, 1w) or absolute (2026-05-21 09:00).', flags: MessageFlags.Ephemeral };
	}
	return { content: 'Failed to set reminder.', flags: MessageFlags.Ephemeral };
}
