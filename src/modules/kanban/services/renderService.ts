...
export function renderBoardEmbed(boardName: string, cardsByColumn: Record<Column, CardData[]>) {
...
export function renderDigestEmbed(boardName: string, cards: CardData[], type: 'daily' | 'weekly') {
	const embed = new EmbedBuilder()
...
export async function sendBoardMessage(channel: TextChannel, embed: EmbedBuilder, existingMessageId?: string) {
...
export async function sendDigestMessage(channel: TextChannel, embed: EmbedBuilder, roleMention?: string) {
	const content = roleMention ? `<@&${roleMention}>` : undefined;
